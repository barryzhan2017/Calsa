    ...
    let rec build_expr builder m global_vars ((_, e) : sexpr) = match e with
      ...
      | SCall ("print", [(t, e)]) ->
        if t = List then
          match e with
          | SId s->
            L.build_call printList_func [| lookup_value m global_vars s |] "" builder
      ...
      | SCall ("get", [(t1, SId s); (t2, e2)]) ->
        if t1 = List then
          ...
          L.build_call getHashtable_func [| lookup_value m global_vars s; (build_expr builder m global_vars (t2, e2)) |]
            "get" builder
        else
