...
and dfs_expr ?fname fncs env e ref_map =
  ...
  let  (fncs', fvs', expr', ref_map') = match expr with
    ...
    | SAssign(s1, e1) ->
      ...
      let left_typ = (StringMap.find s1 env.variables)
      in
      let ref_map'' = (match e1' with 
            (t, SCall(s, expr)) -> StringMap.add s (SFunction (concrete_func2 (left_typ)  expr)) ref_map'
          |_ -> ref_map') in 
          let e2' = (match e1' with 
          (t, SCall(s, expr)) -> (left_typ, SCall(s, expr))
          |_ -> e1') 
          in
      (fncs1, fvs1, (t, SAssign(s1, e2')), ref_map'')
